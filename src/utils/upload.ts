 ...
 */
const upload = (ctx: any, limit?: Options['limits'], fieldname: string = 'headImg'): ApiConfig<string> => {
    const {req, res} = ctx
    // 合并配置
    ...
    multerUp(req, res, function (err) {
        if (err instanceof multer.MulterError) {
            return fail(err.message + '文件大小超出2MB，请重新选择！')
        } else if (err) {
            return fail(err)
        }
        ...
        return success(host + publicPath + relativePath)
    });
    return fail('上传失败')
};
...
const uploadArticleImg = (req: Request, res: Response, limit?: Options['limits'], fieldname: string = 'upload-image') => {
    ...
    ArticleUp(req, res, function (err) {
        if (err instanceof multer.MulterError) {
            return fail(err + '文件大小超出10MB，请重新选择！')
        } else if (err) {
            return fail(err)
        }
